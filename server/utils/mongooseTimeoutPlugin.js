...
module.exports = function timeoutPlugin(schema, options = {}) {
  ...
  const queryMethods = ['find', 'findOne', 'findById', 'countDocuments', 'count', 'distinct'];
  ...
  schema.post(/^find/, function(error, doc, next) {
    if (error) {
      console.error(`Mongoose query error in ${schema.modelName}:`, error.message);
  ...
  schema.statics.getMockData = function() {
    ...
    // Default mock data
    const defaultMock = [
      { _id: `mock-${schema.modelName.toLowerCase()}-1`, createdAt: new Date() },
      { _id: `mock-${schema.modelName.toLowerCase()}-2`, createdAt: new Date(Date.now() - 86400000) }
    ];
