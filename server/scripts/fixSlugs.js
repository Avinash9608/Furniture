...
const fixNullSlugs = async () => {
  ...
  try {
    ...
    const productsWithNullSlugs = await productsCollection
      .find({
    ...
    console.log(
      `Found ${productsWithNullSlugs.length} products with null slugs`
    );
    ...
    for (const product of productsWithNullSlugs) {
      try {
        ...
        if (result.modifiedCount === 1) {
          console.log(
            `Fixed product ${product._id}: "${product.name}" -> slug: "${finalSlug}"`
          );
        } else {
    ...
    if (remainingNullSlugs > 0) {
      console.warn(
        `There are still ${remainingNullSlugs} products with null slugs`
      );
    } else {
