...
exports.getProductById = async (req, res) => {
  console.log(`[DirectProductDetails] Fetching product with ID: ${req.params.id}`);
  ...
  try {
    ...
    console.log(`[DirectProductDetails] Using connection string: ${redactedUri}`);
    ...
    if (/^[0-9a-fA-F]{24}$/.test(productId)) {
      try {
        console.log('[DirectProductDetails] Trying to find product with ObjectId');
        ...
        console.error('[DirectProductDetails] Error finding product with ObjectId:', error.message);
        attempts.push({
          strategy: 'objectId',
          success: false,
          error: error.message
    ...
    if (!product) {
      try {
        console.log('[DirectProductDetails] Trying to find product with string ID');
        ...
        console.error('[DirectProductDetails] Error finding product with string ID:', error.message);
        attempts.push({
          strategy: 'stringId',
          success: false,
          error: error.message
    ...
    if (!product) {
      try {
        ...
        console.error('[DirectProductDetails] Error finding product with slug:', error.message);
        attempts.push({
          strategy: 'slug',
          success: false,
          error: error.message
    ...
    if (!product) {
      try {
        console.log('[DirectProductDetails] Trying to find product with flexible query');
        ...
        if (product) {
          console.log('[DirectProductDetails] Found product with flexible query');
        }
      } catch (error) {
        console.error('[DirectProductDetails] Error finding product with flexible query:', error.message);
        attempts.push({
          strategy: 'flexible',
          success: false,
          error: error.message
    ...
    if (!product) {
      try {
        console.log('[DirectProductDetails] Trying to find any product as fallback');
        ...
        console.error('[DirectProductDetails] Error finding fallback product:', error.message);
        attempts.push({
          strategy: 'anyProduct',
          success: false,
          error: error.message
      ...
      if (product.category) {
        try {
          ...
          if (typeof product.category === 'string') {
            console.log(`[DirectProductDetails] Fetching category for ID: ${product.category}`);
            ...
            if (/^[0-9a-fA-F]{24}$/.test(product.category)) {
              try {
                category = await categoriesCollection.findOne({ _id: new ObjectId(product.category) });
              } catch (categoryError) {
                console.error('[DirectProductDetails] Error fetching category with ObjectId:', categoryError.message);
              }
            }
            ...
            if (!category) {
              try {
                category = await categoriesCollection.findOne({ _id: product.category });
              } catch (categoryError) {
                console.error('[DirectProductDetails] Error fetching category with string ID:', categoryError.message);
              }
            }
            
            // If we found the category, use it
            if (category) {
              console.log(`[DirectProductDetails] Found category: ${category.name}`);
              product.category = {
                _id: category._id,
                name: category.name,
              ...
              if (categoryMap[categoryId]) {
                console.log(`[DirectProductDetails] Using category mapping for ID ${categoryId}: ${categoryMap[categoryId]}`);
                product.category = {
                  _id: categoryId,
                ...
                console.log(`[DirectProductDetails] No mapping found for category ID: ${categoryId}`);
                product.category = {
                  _id: categoryId,
                  name: categoryId.length === 24 ? `Category ${categoryId.substring(0, 8)}` : categoryId,
                  slug: `category-${categoryId.substring(0, 8)}`
                };
              }
            }
          } else if (typeof product.category === 'object' && product.category && !product.category.name) {
            // If category is an object but doesn't have a name, try to add one
            const categoryId = product.category._id ? product.category._id.toString() : '';
            if (categoryId && categoryMap[categoryId]) {
              product.category.name = categoryMap[categoryId];
              product.category.slug = categoryMap[categoryId].toLowerCase().replace(/\s+/g, '-');
            } else {
              product.category.name = categoryId.length === 24 ? `Category ${categoryId.substring(0, 8)}` : 'Unknown Category';
              product.category.slug = `category-${categoryId.substring(0, 8)}`;
            }
          }
        } catch (categoryError) {
          console.error('[DirectProductDetails] Error processing category:', categoryError.message);
    ...
    console.log('[DirectProductDetails] No product found, returning mock product');
    
    // Create a mock product
    const mockProduct = {
      _id: productId,
      name: "Sample Product (Mock)",
      description: "This is a sample product shown when the product could not be found in the database.",
      price: 19999,
      discountPrice: 15999,
      category: {
        _id: "sample-category",
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        specifications: [
          ...
          { name: "Stack", value: error.stack ? error.stack.split('\n')[0] : 'No stack trace' }
        ],
        reviews: [],
    ...
    if (client) {
      try {
        ...
        console.error('[DirectProductDetails] Error closing MongoDB client:', closeError.message);
      }
    }
  }
};
