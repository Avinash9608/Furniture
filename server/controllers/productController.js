  ...
  try {
    ...
    if (req.files && req.files.length > 0) {
      ...
      try {
        ...
        imageUrls = req.files.map(file => getProperImageUrl(file.path.replace(/\\/g, '/').replace('uploads/', '/uploads/')));
    ...
    try {
      ...
      if (result.acknowledged) {
        ...
        const verifiedProduct = await productsCollection.findOne({ _id: result.insertedId });
        ...
        if (verifiedProduct) {
          ...
          try {
            ...
            const category = await categoriesCollection.findOne({ _id: new ObjectId(productData.category) });
...
const getAllProducts = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const productsCollection = db.collection('products');
      products = await productsCollection.find({}).sort({ createdAt: -1 }).toArray();
...
const getProductById = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      // Process image URLs to ensure they're proper
      product.images = (product.images || []).map(image => getProperImageUrl(image));
...
const updateProduct = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      let existingProduct;
      try {
        existingProduct = await productsCollection.findOne({ _id: new ObjectId(id) });
      } catch (idError) {
      ...
      if (req.files && req.files.length > 0) {
        ...
        try {
          ...
          const newImageUrls = req.files.map(file => 
            getProperImageUrl(file.path.replace(/\\/g, '/').replace('uploads/', '/uploads/'))
          );
        ...
        // If image URLs are provided directly in the request body
        try {
          const parsedImages = JSON.parse(req.body.images);
          imageUrls = Array.isArray(parsedImages) ? parsedImages : [parsedImages];
        } catch (e) {
          // If not JSON, treat as comma-separated string
          imageUrls = req.body.images.split(',').map(url => url.trim());
        }
      }
      ...
      if (result.modifiedCount === 1) {
        ...
        updatedProduct = await productsCollection.findOne({ _id: existingProduct._id });
        
        // Process image URLs to ensure they're proper
        updatedProduct.images = (updatedProduct.images || []).map(image => getProperImageUrl(image));
...
const deleteProduct = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      // Delete the product
      const result = await productsCollection.deleteOne({ _id: existingProduct._id });
        ...
        for (const imageUrl of existingProduct.images) {
          try {
            ...
            if (imageUrl && imageUrl.includes('cloudinary.com')) {
              ...
              if (publicId) {
                await cloudinary.uploader.destroy(`furniture_products/${publicId}`);
