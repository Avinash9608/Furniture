...
export const getCategoryImage = (category) => {
  ...
  if (category.image) {
    ...
    if (category.image.startsWith("http")) {
      ...
      return `${import.meta.env.VITE_API_BASE_URL || "http://localhost:5000"}${category.image}`;
    }
  }
  
  // If no image is available, use a placeholder with the category name
  return `https://placehold.co/300x300/gray/white?text=${encodeURIComponent(category.name || "Category")}`;
};
