  ...
  for (const endpoint of endpoints) {
    try {
      ...
      if (response && response.data && response.data.success) {
        console.log(`Successfully created product using ${endpoint} endpoint:`, response.data);
        return response.data;
      } else {
        console.warn(`Endpoint ${endpoint} returned success: false or invalid response:`, response.data);
      }
    } catch (error) {
      console.error(`Error creating product with ${endpoint} endpoint:`, error.response?.data || error.message);
      lastError = error;
  ...
  try {
    ...
    for (const endpoint of endpoints) {
      try {
        ...
        console.error(`Error getting products with ${endpoint} endpoint:`, error.response?.data || error.message);
        lastError = error;
      }
    }
