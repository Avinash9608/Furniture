  ...
  useEffect(() => {
    const fetchProductDetails = async () => {
      try {
        ...
        console.log(`ProductDetailFallback: Fetching product details for ID: ${id}`);
        
        // Strategy 1: Check if we have preloaded product data from the server
        if (window.__PRELOADED_PRODUCT_DATA__ && window.__PRELOADED_PRODUCT_DATA__.data) {
          ...
          try {
            ...
            if (response && response.data) {
              if (response.data.data) {
                ...
                console.log("ProductDetailFallback: Product data received from standard format");
                break;
              } else if (response.data.success && response.data.data) {
                // Direct API response format
                productData = response.data.data;
                endpointSource = `${endpoint} (direct format)`;
                console.log("ProductDetailFallback: Product data received from direct API format");
                break;
              } else if (typeof response.data === 'object' && response.data._id) {
                // Direct object response
                productData = response.data;
                endpointSource = `${endpoint} (object format)`;
                console.log("ProductDetailFallback: Product data received as direct object");
                break;
              } else if (response.data.formats) {
                // Debug endpoint format
                productData = response.data.data;
                endpointSource = `${endpoint} (debug format)`;
                console.log("ProductDetailFallback: Product data received from debug endpoint");
                break;
              }
            }
            
            console.log(`ProductDetailFallback: Endpoint ${endpoint} returned invalid data format`);
          } catch (endpointError) {
            console.error(`ProductDetailFallback: Error with endpoint ${endpoint}:`, endpointError.message);
          }
        }
        
        // If we found product data from any endpoint, use it
        if (productData) {
          console.log(`ProductDetailFallback: Successfully fetched product data from ${endpointSource}`);
        ...
        console.warn("ProductDetailFallback: All endpoints failed, using mock product");
        ...
        const mockProduct = {
          ...
          ratings: 4.5,
          numReviews: 12,
          images: ["https://placehold.co/800x600/orange/white?text=Fallback+Product"],
          specifications: [
        ...
        setError("Failed to load product details from all sources. Showing a fallback product.");
        setLoading(false);
