  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      try {
        ...
        response = await fetch(regularApiUrl, {
          method: "POST",
        ...
        // If the regular endpoint fails, try the direct login endpoint
        if (!response.ok && response.status === 500) {
          console.log("Regular admin login failed with server error, trying direct login");
          throw new Error("Server error, trying direct login");
        }
      } catch (error) {
        console.log("Falling back to direct admin login endpoint:", directApiUrl);
